...
const PrivilegesTpes = () => {
  ...
  const GetPrivelegesTypes = async () => {
    const res = await fetch("/getprivilegestypes", {
      method: "POST",
      headers: {
        ...
        Object.keys(privilegesTypes[item]).map((item2) => {
          if (item2 === keys) {
            ...
            console.log("Item value = ", {
              ...privilegesTypes,
              [item]: {...privilegesTypes[item] ,[item2]: privilegesTypes[item][item2],  },
            });
            d2 = {
              ...privilegesTypes,
              [item]: {...privilegesTypes[item], [item2]: privilegesTypes[item][item2],  },
 ...
 setPrivilegesTypes(d2);
  ...
  {
    const res = await fetch("/updateprivilegestypes", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <>
      <Grid>
        ...
        <center><h1>PrivilegesTpes</h1></center>
          ...
          {privilegesTypes &&
            ...
            Object.keys(privilegesTypes).map((key, index) => (
              <>
                  ...
                  <h4 style={{backgroundColor:"#3F51B5" ,color:"white" ,padding:5}}> {key}</h4>
                ...
                {privilegesTypes &&
                  ...
                  <FormControl component="fieldset" style={{justifyContent:"flex-end", display:'flex'}} >
                        ...
                        <RadioGroup
                          ...
                          onChange={(e)=>{HandleInput(key,key2,e.target.value)}}
          ...
          <Button fullWidth color="primary" variant="contained" onClick={SubmitPrivilegeTypes}>
            Submit
          </Button>
        </Paper>
      </Grid>
